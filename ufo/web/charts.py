...
# Fallback colours written into the SVG itself. CSS (style.css) overrides them
# with themed values; they only show if the stylesheet is missing or stale,
# so charts never fall back to SVG's default black.
FILL = {"bar": "#2a78d6", "bar hot": "#eb6834"}
RAMP = ["#efeee9", "#cde2fb", "#9ec5f4", "#6da7ec", "#3987e5", "#256abf", "#184f95"]
...
def responsive(wide: Markup, narrow: Markup) -> Markup:
    """Both renderings; CSS shows the one that suits the screen width."""
    # the narrow copy is also hidden inline so a stale stylesheet shows one chart
    return Markup(f'<div class="only-wide">{wide}</div><div class="only-narrow" style="display:none">{narrow}</div>')


def timeline(years: list[dict], waves: list[dict], link: str = "/patterns/evidence?year={year}",
             narrow: bool = False) -> Markup:
    """Reports per year with wave periods shaded and labelled.

    ``narrow`` draws a phone-sized version (text stays legible at ~360px)."""
    if not years:
        return Markup('<p class="muted small">No dated reports yet.</p>')
    if narrow:
        w, h, top, bottom, left, gap, tick, fs, char = 400, 250, 64, 22, 26, 0.6, 20, 11, 3.3
    else:
        w, h, top, bottom, left, gap, tick, fs, char = 1000, 244, 48, 26, 30, 2, 10, 12, 3.6
    n = len(years)
    step = (w - left) / n
    bw = max(1.0, step - gap)
    ...
    ch = h - top - bottom
    first = years[0]["year"]
    parts = [f'<svg class="chart timeline{" narrow" if narrow else ""}" viewBox="0 0 {w} {h}" role="img" aria-label="Reports per year">']
    # gridlines
    for frac in (0.5, 1.0):
        y = top + ch * (1 - frac)
        parts.append(f'<line class="grid" stroke="#e4e2db" x1="{left}" x2="{w}" y1="{y:.1f}" y2="{y:.1f}"/>')
        parts.append(f'<text class="axis" fill="#77766f" font-size="11" x="{left - 6}" y="{y + 4:.1f}" text-anchor="end">{round(mx * frac)}</text>')
    ...
    for wv in waves:
        ...
        cx, half = (x0 + x1) / 2, len(label) * char + 4
        cx = min(max(cx, left + half), w - half)
        # stagger labels that would collide onto up to three rows
        row = next((r for r, end in enumerate(rows_end) if cx - half > end), len(rows_end) - 1)
        rows_end[row] = cx + half
        ly = top - 8 - row * 16
        parts.append(f'<rect class="wave-band" fill="#eb6834" fill-opacity="0.1" x="{x0:.1f}" y="{ly - 12}" width="{x1 - x0:.1f}" height="{top + ch - ly + 12}" rx="4"/>')
        parts.append(f'<text class="wave-label" fill="#52514e" font-size="{fs}" font-weight="600" x="{cx:.1f}" y="{ly}" text-anchor="middle">{label}</text>')
    for i, y in enumerate(years):
        ...
        parts.append(
            f'<a href="{href}"><rect class="{cls}" fill="{FILL[cls]}" x="{x:.1f}" y="{yy:.1f}" width="{bw:.1f}" height="{bh:.1f}" rx="{min(2, bw / 3):.1f}">'
            f'<title>{y["year"]}: {c:.0f} report{"s" if round(c) != 1 else ""}</title></rect></a>'
        )
    parts.append(f'<line class="baseline" stroke="#77766f" stroke-opacity="0.6" x1="{left}" x2="{w}" y1="{top + ch}" y2="{top + ch}"/>')
    for i, y in enumerate(years):
        if y["year"] % tick == 0:
            x = left + i * step + step / 2
            parts.append(f'<text class="axis" fill="#77766f" font-size="11" x="{x:.1f}" y="{h - 8}" text-anchor="middle">{y["year"]}</text>')
    parts.append("</svg>")
...
def month_bars(counts: list[float], peak: int | None = None, label: str = "") -> Markup:
    ...
    step = w / 12
    parts = [f'<svg class="chart months" style="max-width:280px;width:100%" viewBox="0 0 {w} {h}" role="img" aria-label="{_e(label)} reports by month">']
    for i, c in enumerate(counts):
        bh = (h - pad - 6) * c / mx if c else 0
        x = i * step + 2
        cls = "bar hot" if peak == i + 1 else "bar"
        if bh:
            parts.append(f'<rect class="{cls}" fill="{FILL[cls]}" x="{x:.1f}" y="{h - pad - bh:.1f}" width="{step - 4:.1f}" height="{bh:.1f}" rx="2">'
                         f'<title>{MONTH_NAMES[i]}: {c:.0f}</title></rect>')
        parts.append(f'<text class="axis" fill="#77766f" font-size="11" x="{x + (step - 4) / 2:.1f}" y="{h - 2}" text-anchor="middle">{MONTHS[i]}</text>')
    parts.append(f'<line class="baseline" stroke="#77766f" stroke-opacity="0.6" x1="0" x2="{w}" y1="{h - pad}" y2="{h - pad}"/></svg>')
    return Markup("".join(parts))
...
def decade_spark(decades: dict[str, int], all_decades: list[str]) -> Markup:
    ...
    for i, d in enumerate(all_decades):
        ...
        if bh:
            parts.append(f'<rect class="bar" fill="#2a78d6" x="{i * step + 1:.1f}" y="{h - bh:.1f}" width="{step - 2:.1f}" height="{bh:.1f}" rx="1.5">'
                         f'<title>{d}: {c}</title></rect>')
    parts.append(f'<line class="baseline" stroke="#77766f" stroke-opacity="0.6" x1="0" x2="{w}" y1="{h - 0.5}" y2="{h - 0.5}"/></svg>')
    return Markup("".join(parts))
...
def us_tilemap(grid: list[dict], link: str = "/patterns/evidence?place=US-{code}") -> Markup:
    ...
    for g in grid:
        ...
        inner = (f'<rect class="state q{q}" fill="{RAMP[q]}" x="{x}" y="{y}" width="{size}" height="{size}" rx="6"><title>{_e(tip)}</title></rect>'
                 f'<text class="state-label l{q}" fill="{"#ffffff" if q >= 4 else "#0b0b0b"}" font-size="13" font-weight="600" x="{x + size / 2}" y="{y + size / 2 + 4}" text-anchor="middle">{g["code"]}</text>')
...
def strip_plot(rows: list[dict], lo: int = 1940, hi: int = 2026, narrow: bool = False) -> Markup:
    """One row per release; each record is a dot at its incident year."""
    left, w, rh, tick, r = (40, 400, 32, 20, 2.8) if narrow else (150, 1000, 34, 10, 3.5)
    h = rh * len(rows) + 26
    span = hi - lo
    rng = random.Random(3)
    parts = [f'<svg class="chart strip{" narrow" if narrow else ""}" viewBox="0 0 {w} {h}" role="img" aria-label="Incident years of the records in each release">']
    for dec in range(lo, hi + 1, tick):
        x = left + (w - left - 10) * (dec - lo) / span
        parts.append(f'<line class="grid" stroke="#e4e2db" x1="{x:.1f}" x2="{x:.1f}" y1="0" y2="{h - 22}"/>'
                     f'<text class="axis" fill="#77766f" font-size="11" x="{x:.1f}" y="{h - 6}" text-anchor="middle">{dec}</text>')
    for i, row in enumerate(rows):
        cy = i * rh + rh / 2
        parts.append(f'<text class="row-label" fill="#52514e" font-size="12" x="{left - 12}" y="{cy + 4:.1f}" text-anchor="end">{_e(row["short"] if narrow else row["label"])}</text>')
        for d in row["docs"]:
            if not d["year"] or d["year"] < lo:
                continue
            x = left + (w - left - 10) * (d["year"] - lo) / span
            y = cy + rng.uniform(-rh * 0.3, rh * 0.3)
            parts.append(f'<a href="/documents/{d["id"]}"><circle class="dot" fill="#2a78d6" fill-opacity="0.55" cx="{x:.1f}" cy="{y:.1f}" r="{r}">'
